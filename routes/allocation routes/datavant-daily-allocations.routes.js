// routes/datavant-daily-allocations.routes.js - Datavant Daily Allocation CRUD
const express = require('express');
const router = express.Router();
const mongoose = require('mongoose');
const DatavantDailyAllocation = require('../../models/DatavantDailyAllocation');
...
router.post('/', authenticateResource, async (req, res) => {
  try {
    ...
    // Verify resource has access to this Datavant location
    ...
    const entryRecordCount = parseInt(record_count) || 1;
    ...
    const allocation = new DatavantDailyAllocation({
      ...
      record_count: entryRecordCount,
      remark: remark || '',
      billing_rate: 0, // Configure as needed
      billing_amount: 0,
      is_locked: false
    });
    ...
    res.status(201).json({
      message: 'Datavant allocation entry created successfully',
      allocation
    });

  } catch (error) {
    console.error('Error creating Datavant allocation:', error);
...
router.get('/my-allocations', authenticateResource, async (req, res) => {
  try {
    ...
    const [allocations, total] = await Promise.all([
      DatavantDailyAllocation.find(filter)
        .sort({ allocation_date: -1, sr_no: 1 })
        .skip(skip)
        .limit(parseInt(limit))
        .lean(),
      DatavantDailyAllocation.countDocuments(filter)
    ]);
    ...
    console.error('Error fetching Datavant allocations:', error);
...
router.put('/:id', authenticateResource, async (req, res) => {
  try {
    const allocation = await DatavantDailyAllocation.findById(req.params.id);
    ...
    // Allowed updates
    const { request_type, data_source, record_count, remark } = req.body;
    
    if (request_type !== undefined) allocation.request_type = request_type;
    if (data_source !== undefined) allocation.data_source = data_source;
    if (record_count !== undefined) allocation.record_count = parseInt(record_count) || 1;
    ...
    console.error('Error updating Datavant allocation:', error);
...
router.delete('/:id', authenticateResource, async (req, res) => {
  try {
    ...
    console.error('Error deleting Datavant allocation:', error);
...
router.get('/admin/all', authenticateUser, async (req, res) => {
  try {
    const { date, start_date, end_date, resource_id, subproject_id, page = 1, limit = 100 } = req.query;
    ...
    console.error('Error fetching all Datavant allocations:', error);
    res.status(500).json({ message: 'Error fetching allocations', error: error.message });
  }
});
